...
# 根据输入获取相应的角色列表
def get_hero_list_by_input(input):
    frame.grid(row=1, column=0, )
    hero_list_select = []
    if input:
        for i in hero_info_list:
            if i.get('name').find(input) != -1:
                hero_list_select.append(i)
        return hero_list_select
    else:
        return
...
# func_list = [display_all, display_all, display_all, display_all, display_all, display_all, display_all]
func_arg_list = ['is_tank', 'is_tank', 'is_tank', 'is_tank', 'is_tank', 'is_tank', 'all']
...
vbar.grid(row=1,column=9,rowspan=8,columnspan=10)
...
all = Button(root, text='全部', command=lambda: display_hero('all')).grid(row=0, column=0, padx=50)
is_tank = Button(root, text='坦克', command=lambda: display_hero('is_tank')).grid(row=0, column=1, padx=50)
is_warrior = Button(root, text='战士', command=lambda: display_hero('is_warrior')).grid(row=0, column=2, padx=50)
is_assa = Button(root, text='刺客', command=lambda: display_hero('is_assa')).grid(row=0, column=3, padx=50)
is_master = Button(root, text='法师', command=lambda: display_hero('is_master')).grid(row=0, column=4, padx=50)
is_archer = Button(root, text='射手', command=lambda: display_hero('is_archer')).grid(row=0, column=5, padx=50)
is_sup = Button(root, text='辅助', command=lambda: display_hero('is_sup')).grid(row=0, column=6, padx=50)
entry = Entry(root)
